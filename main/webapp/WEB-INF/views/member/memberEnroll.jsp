	...
	<form name="memberEnrollFrm" 
		...
		<table>
			<tr>
				...
				<input type="hidden" id="idValid" name ="inValid" value="0" />
				<%-- 사용가능한 아이디인 경우1, 이미 사용중인 아이디인 경우 0  --%>
				</td>
			</tr>
			<tr>
				<th>패스워드<sup>*</sup></th>
			...
			<tr>
				<th>이름<sup>*</sup></th>
				<td>	
				<input type="text"  name="memberName" id="memberName" value="길동"required><br>
				</td>
			</tr>
			<tr>
				<th>생년월일</th>
				<td>	
				<input type="date" name="birthday" id="birthday"  value ="1977-07-07"><br />
				</td>
			</tr> 
			<tr>
				<th>이메일</th>
				<td>	
					<input type="email" placeholder="abc@xyz.com" name="email" id="email" value ="ghonddg@naver.com"><br>
				</td>
			</tr>
			<tr>
				<th>휴대폰<sup>*</sup></th>
...
const checkIdDuplicate = () =>{
	const memberId = document.querySelector("#_memberId");
	  if(!/^[A-Za-z0-9]{4,}$/.test(memberId.value)){
		...
		//frm의 action 주소를 사용하기 때문에 open의 url은 비워둔다.
...
document.memberEnrollFrm.onsubmit = (e) => {
    ...
    if(idValid.value !== '1'){
    	alert("아이디 중복검사 해주세요.");
    	memberId.nextSiblingElement.focus(); //중복검사 버튼 
    	return false;
    }

    // 비밀번호 - 영문자/숫자/특수문자 !@#$% 4글자이상
    if(!/^[A-Za-z0-9!@#$%]{4,}$/.test(password.value)){
        alert("비밀번호는 영문자/숫자/특수문자!@#$% 구성된 4글자 이상이어야 합니다.")
        password.select();
        return false;
