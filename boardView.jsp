	...
	<div class="comment-container">
		...
		<%
				for(BoardComment bc : comments){
					boolean cancommentEdit = loginMember != null && 
							(loginMember.getMemberRole() == MemberRole.A ||
		...
		%>
			...
			<tr class="level1">
				...
				<%	
				if(cancommentEdit){ %>
					<button class="btn-delete" name = "no" value="<%= bc.getNo() %>">삭제</button>
					<% } %>
				</td>
			</tr>
		<%
	...
	button.onclick = (e) => {
		console.log(e.target.value);
		   let f = document.createElement('form');
		    ...
		    obj.setAttribute('value', e.target.value);
		    
		    f.appendChild(obj);
		    f.setAttribute('method', 'post');
		    f.setAttribute('action', '<%= request.getContextPath() %>/board/boarddeletecomment');
...
<form action="<%= request.getContextPath() %>/board/boarddeletecomment" name="bcdFrm" method="POST">
	<input type="hidden" name="boardno" value="<%= board.getNo() %>" />
</form>
<script>
